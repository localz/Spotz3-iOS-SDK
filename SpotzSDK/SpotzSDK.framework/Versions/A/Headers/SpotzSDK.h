...
/**
 *  SpotzSDK is the main class that handles the integration between the client and Localz's Spotz Platform.
 */
 ...
 *
 *  @return Retuns an array of SpotzData at the current site. Returns nil if closest site is unknown or spots have not yet been retrieved.
 */
 ...
 *  @return         Returns true if inside, false if outside or not yet detected.
 */
 ...
 *
 *  @param rating     A numerical rating value.
 ...
 *  @param module     (optional) the module name associated with this rating. e.g. Retail Module, Click&Collect, etc. For internal use only.
 ...
 *  @param attributes   Any additional information to be associated with this device. Useful when passing additional info to third party extensions.
 ...
 *
 *  @param completion   A block object with no return value, and two arguments: enable is true if the service is on, false if not, and state which gives more information about the status.
 *  @return             Retuns true if the service is on, false if not.
 */
 ...
 *
 *  @param completion   A block object with no return value, and two arguments: enable is true if the service is on, false if not, and state which gives more information about the status.
 *  @return             Returns true if it is on, false if not.
 */
